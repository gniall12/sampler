...
export class VolumeModalComponent implements OnInit {
  ...
  noteLengthOptions: Options = {
    ...
    translate: (value: number) => {
      return this.noteLengthMappings[value];
    },
  };
  volume: number;
  reverb: number;
  ...
  public volumeChange() {
    if (this.volumeChanging) clearTimeout(this.volumeChanging);
    const thisClass = this;
    this.volumeChanging = setTimeout(function () {
      thisClass.samplesService.setVolume(
        thisClass.map["key"],
        thisClass.volume
      );
  ...
  public reverbChange() {
    if (this.reverbChanging) clearTimeout(this.reverbChanging);
    const thisClass = this;
    this.reverbChanging = setTimeout(function () {
      thisClass.samplesService.setReverb(
        thisClass.map["key"],
        thisClass.reverb
      );
      thisClass.samplesService.play(thisClass.map["key"], null);
  ...
  public delayChange() {
    if (this.delayChanging) clearTimeout(this.delayChanging);
    const thisClass = this;
